...
async def upload_to_imgbb(image_data: str, name: str = None, expiration: int = None) -> dict:
    ...
    if name:
        post_data["name"] = name
    async with aiohttp.ClientSession() as session:
        async with session.post(IMGBB_URL, params=params, data=post_data, timeout=aiohttp.ClientTimeout(total=30)) as resp:
            resp.raise_for_status()
...
async def cmd_start(client: Client, msg: Message):
    ...
    welcome_text = (
        ...
        f"{E_CROWN} <b>Session:</b> {msg.from_user.first_name} (ID: {msg.from_user.id})"
    )
    try:
...
async def about_cb(client: Client, cb: CallbackQuery):
    text = (
        ...
        f"{E_GEAR} <b>Version:</b> 4.0.0 (Pyrogram)\n"
