...
v4.0.0 — Pyrogram + ButtonStyle colored buttons
"""
...
user_state:  dict[int, str] = {}   # "image" | "url" | None
...
async def cmd_start(client: Client, msg: Message):
    ...
    welcome_text = (
        ...
        f"{E_CROWN} <b>Session:</b> {msg.from_user.first_name} (<code>{msg.from_user.id}</code>)"
    )
...
async def cmd_help(client: Client, msg: Message):
    ...
    await msg.reply(
        f"{E_INFO} <b>How To Use</b>\n\n"
        f"{E_ARROW} 1️⃣ Send any image (photo or file)\n"
        f"{E_ARROW} 2️⃣ I upload it to ImgBB\n"
        f"{E_ARROW} 3️⃣ You get a permanent direct URL!\n\n"
        f"<blockquote>{E_GEAR} Formats: JPG, PNG, BMP, GIF, TIFF, WEBP\n"
        f"{E_DIAMOND} Max: 32MB</blockquote>\n\n"
...
async def about_cb(client: Client, cb: CallbackQuery):
    text = (
        ...
        f"<blockquote>{E_ROCKET} <b>Version:</b> 4.0.0 (Pyrogram)\n"
        f"{E_GEAR} <b>Library:</b> Pyrogram async\n"
        f"{E_SHIELD} <b>Features:</b> Wallhaven wallpapers + ImgBB hosting\n"
        f"{E_STAR} <b>Colored buttons:</b> ✅ Supported</blockquote>\n\n"
...
async def back_start_cb(client: Client, cb: CallbackQuery):
    ...
    try:
        ...
        await cb.edit_message_caption(caption=welcome_text, reply_markup=start_inline_kb(), parse_mode=ParseMode.HTML)
    await cb.answer()
...
async def cmd_cancel(client: Client, msg: Message):
    user_state.pop(msg.from_user.id, None)
    await msg.reply(f"{E_CHECK} Cancelled. Main menu:", reply_markup=main_kb(), parse_mode=ParseMode.HTML)
