...
# ✅ FIXED: data= use kar raha hai files= ki jagah (400 Bad Request fix)
async def upload_to_imgbb(image_data: str, name: str = None, expiration: int = None) -> dict:
    ...
    # ✅ FIX: form data (data=) use karo, multipart files= nahi
    # ImgBB expects base64 string as plain form field, not multipart upload
    ...
    resp = await loop.run_in_executor(
        None,
        lambda: _requests.post(IMGBB_URL, params=params, data=form, timeout=30)
    )
    resp.raise_for_status()
    data = resp.json()
